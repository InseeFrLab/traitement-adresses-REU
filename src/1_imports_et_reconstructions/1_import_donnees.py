...
csv_files = list(filter(lambda f: f.endswith(".csv"), os.listdir(rawdatapath)))

# Importer tous les CSV
list_df = [
    pd.read_csv(rawdatapath + file, on_bad_lines="skip", header=0, dtype=str, sep=",")
...
df.to_parquet(datapath + "adressesREU_brutes.parquet", compression="gzip")
...
# Sauvegarder la table complète en parquet
df_BAN_adresses.to_parquet(datapath + "adressesBAN_brutes.parquet", compression="gzip")
...
df_BAN_lieuxdits.to_parquet(
    datapath + "lieuxditsBAN_brutes.parquet", compression="gzip"
)
